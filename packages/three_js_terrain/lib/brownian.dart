...
class Brownian{
  Brownian(Float32List g, TerrainOptions options){
    List<double> untouched = [],
      ...
      double current = vertices[j * xl + i];
    ...
    current = math.Random().nextDouble() * (options.maxHeight! - options.minHeight!) + options.minHeight!;
    touched.add(current);
    ...
    while (touched.length != numVertices) {
      ...
      for (n = -1; n <= 1; n++) {
        for (m = -1; m <= 1; m++) {
          key = (j+n)*xl + i + m;
          if (touched.indexOf(vertices.get(key)) == -1 && i+m >= 0 && j+n >= 0 && i+m < xl && j+n < yl && n != 0 && m != 0) {
            untouched.add(vertices.get(key));
          }
        }
      }
      ...
      if (math.Random().nextDouble() < changeDirectionProbability) {
        current = untouched.removeAt((math.Random().nextDouble() * untouched.length).floor());//[0];
      ...
      if (c != 0) {
        ...
        current = sum / c + Easing.easeInWeak(lastAdjust) * maxHeightAdjust * 2 - maxHeightAdjust;
      }
      touched.add(current);
    }
