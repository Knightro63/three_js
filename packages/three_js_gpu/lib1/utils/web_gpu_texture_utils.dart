...
class WebGPUTextureUtils {
  ...
  DepthTexture depthTexture = DepthTexture();
	...
	/// @return {GPUTexture} The GPU texture.
	///
	GPUTexture _getDefaultCubeTextureGPU( String format ) {
	...
	///
	Map _getBlockData( String format ) {
	...
	///
	String _convertAddressMode( int value ) {

		String addressMode = GPUAddressMode.clampToEdge;
	...
	///
	String _convertFilterMode( int value ) {

		String filterMode = GPUFilterMode.linear;
	...
	///
	int _getBytesPerTexel( String format ) {
	...
	///
	Type _getTypedArrayType( String format ) {
		...
		if ( format == GPUTextureFormat.R8Snorm ) return Int8List;
...
///
String getFormat(Texture texture, { GPUDevice? device } ) {
