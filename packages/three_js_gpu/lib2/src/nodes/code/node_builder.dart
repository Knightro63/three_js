...
import '../../renderers/common/bind_group.dart';
import '../../renderers/common/chain_map.dart';
import '../../renderers/common/clipping_context.dart';
import '../../renderers/common/nodes/node_uniform.dart';
import '../core/node.dart';
import '../core/node_uniform.dart';
import 'node_parser.dart';

final rendererCache = new WeakMap();

final Map<Type, String> typeFromArray = {
  Int8List: 'int',
  Int16List: 'int',
  Int32List: 'int',
  Uint8List: 'uint',
...
String toFloat(dynamic value) {
  double numValue = double.parse(value.toString());
  return numValue.toStringAsFixed(numValue % 1 == 0 ? 1 : 1);
}
...
abstract class NodeBuilder {
  ...
  String? computeShader = null;
  Map<String,List<Node>> flowNodes = { 'vertex': [], 'fragment': [], 'compute': [] };
  Map<String,String> flowCode = { 'vertex': '', 'fragment': '', 'compute': '' };
  Map<String,dynamic> uniforms = { 'vertex': [], 'fragment': [], 'compute': [], 'index': 0 };
  Map<String,dynamic> structs = { 'vertex': [], 'fragment': [], 'compute': [], 'index': 0 };
  Map<String,dynamic> bindings = { 'vertex': {}, 'fragment': {}, 'compute': {} };
  Map<String,dynamic> bindingsIndexes = {};
  List<BindGroup>? bindGroups = null;
  List<NodeAttribute> attributes = [];
  List<NodeAttribute> bufferAttributes = [];
  List<NodeVarying> varyings = [];
  Map<String,List<NodeCode>> codes = {};
  Map<String,dynamic> vars = {};
  Map<String,dynamic> declarations = {};
  Map<String,String> flow = { 'code': '' };
  List<Node> chaining = []; 
  StackNode stack; // Initialized in constructor
  List<StackNode> stacks = []; 
  String tab = '\t'; 
  FunctionNode? currentFunctionNode; 
  Map<String, dynamic> context = {'material': this.material}; // Initialized in constructor
  NodeCache cache = NodeCache(); 
  NodeCache globalCache; // Initialized in constructor
  WeakMap flowsData = WeakMap(); 
  String? shaderStage; 
  String? buildStage; 
  List<SubBuildNode> subBuildLayers = []; 
  StackNode? currentStack; 
  String? subBuildFn;

	NodeBuilder(this.object, this.renderer, this.parser ) {
	 ...
	 */
	ChainMap getBindGroupsCache() {
		dynamic bindGroupsCache = rendererCache.get( this.renderer );

		if ( bindGroupsCache == null ) {
	...
	RenderTarget createRenderTarget(int width, int height, [RenderTargetOptions? options] ) {
		return RenderTarget( width, height, options );
	}
	 ...
	 */
	CubeRenderTarget createCubeRenderTarget(int size, [RenderTargetOptions? options] ) {
		return CubeRenderTarget( size, options );
	}
	...
	getBindGroupArray( groupName, shaderStage ) {
		...
		if ( bindGroup == null ) {
			...
			if ( this.bindingsIndexes[ groupName ] == null ) {
	...
	void removeChain(Node node ) {
		final lastChain = this.chaining.removeLsat();

		if ( lastChain != node ) {
			throw ( 'NodeBuilder: Invalid node chaining!' );
		}
	}
	...
	getCacheFromNode( node, parent = true ) {
		...
		if ( data.cache == null ) data.cache = new NodeCache( parent ? this.getCache() : null );
	...
	hasGeometryAttribute( name ) {
		...
		return this.geometry && this.geometry.getAttribute( name ) != null;
	...
	getDataFromNode( node, shaderStage = this.shaderStage, cache = null ) {
		...
		if ( subBuild ) {
			...
			if ( data.subBuildsCache == null ) data.subBuildsCache = {};
	 ...
	 */
	NodeVar getVarFromNode(VarNode node, [String? name, type = node.getNodeType( this ), shaderStage = this.shaderStage, bool readOnly = false ]) {
	 ...
	 */
	NodeBuilder addLineFlowCode(String code, [Node? node] ) {
		if ( code == '' ) return this;
	 ...
	 */
	String getVaryings(String shaderStage );
	 ...
	 */
	String getVar(String  type, String name, [int? count]) {
	 ...
	 */
	String getUniforms(String shaderStage ) {
	 ...
	 */
	String? setShaderStage( shaderStage ) {
	 ...
	 */
	void setBuildStage(String? buildStage ) {
	 ...
	 */
	void addSubBuild(SubBuildNode subBuild ) {
	 ...
	 */
	String? getClosestSubBuild(dynamic data ) {
		var subBuilds;

		if ( data != null && data is Node ) {
			if ( data is ShaderCallNodeInternal ) {
				subBuilds = data.shaderNode.subBuilds;
			} 
      else if ( data is StackNode ) {
		...
		final subBuildLayers = this.subBuildLayers;

		for ( int i = subBuilds.length - 1; i >= 0; i -- ) {
			final subBuild = subBuilds[ i ];
	 ...
	 */
	String getSubBuildOutput(Node node ) {
	 ...
	 */
	String getSubBuildProperty([String? property = '', Node? node] ) {
		String subBuild;
    ...
    final renderer = this.renderer;
		...
		for ( final buildStage in defaultBuildStages ) {
			this.setBuildStage( buildStage );
	 ...
	 */
	NodeUniform getNodeUniform(NodeUniform uniformNode, String type ) {
