...
import '../../renderers/common/nodes/node_builder_state.dart';
...
class Node<T> extends EventDispatcher {
  String? nodeType;
  ...
  /// @yields {Node} A child node.
	getChildren() {
		for ( final childNode in getNodeChildren( this ).childNode ) {
			yield childNode;
		}
	}
			...
			if ( isGenerateOnce ) {
				...
				if ( result == null ) {
					...
					if ( nodeData.generated == null ) {
						...
						console.warning( 'THREE.Node: Recursion detected. $this' );

						result = '/* Recursion detected. */';

					}

				} else if ( nodeData.flowCodes != null && builder.context.nodeBlock != null ) {
  ...
  /// @param {Object} json - The output JSON object.
	void serialize(Map<String,dynamic> json ) {
		final nodeChildren = this.getSerializeChildren();
		final inputNodes = {};

		for ( final nodeChild in nodeChildren ) {
      final property = nodeChild.property;
      final index = nodeChild.index;
      final childNode = nodeChild.childNode;

			if ( index != null ) {
				if ( inputNodes[ property ] == null ) {
					inputNodes[ property ] = int.tryParse( index ) != null? [] : {};
				}
  ...
  /// @return {Object} The serialized node.
    ...
    final type = this.type;
		...
		Map<String,dynamic>? data = meta['nodes'][ uuid ];
