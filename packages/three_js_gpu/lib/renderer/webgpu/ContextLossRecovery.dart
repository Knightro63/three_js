...
class ContextLossRecovery {
  ...
  Future<RecoveryStats> recover(GpuDevice device) async {
    if (!_isRecovering) {
      ...
      try {
        switch (descriptor) {
          case BufferDescriptorHolder(descriptor: final desc):
            final buffer = WebGPUBuffer(device, desc);
            buffer.create();
            buffersRecreated++;
            break;
            
          case TextureDescriptorHolder(descriptor: final desc):
            final texture = WebGPUTexture(device, desc);
            texture.create();
            texturesRecreated++;
            break;
            
          case PipelineDescriptorHolder(descriptor: final desc):
            final pipeline = WebGPUPipeline(device, desc);
            pipeline.create();
            pipelinesRecreated++;
            break;
        }
  ...
  Future<void> monitorDevice(GpuDevice device) async {
    try {
