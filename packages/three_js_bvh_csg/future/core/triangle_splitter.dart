import "package:three_js_bvh_csg/core/utils/extended_triangle.dart";
...
class TriangleSplitter {
  ...
  void splitByTriangle(Triangle triangle) {
    ...
    if ((1.0 - (_triangleNormal.dot(normal)).abs()).abs() < parallelEpsilon) {
      coplanarTriangleUsed = true;

      for (int i = 0, l = triangles.length; i < l; i++) {
        // coplanarCount doesn't seem to exist for Triangle from ThreeJS, and
        // t doesn't seem to serve a function for this function; removed
        // var t = triangles[i];
        // t.coplanarCount = 0;
      }
  ...
  void splitByPlane(Plane plane, clippingTriangle) {
    ...
    for (int i = 0, l = triangles.length; i < l; i++) {
      ...
      if (!_splittingTriangle.intersectsTriangle(tri, target: _edge, suppressLog: true)) {
        continue;
      }
      ...
      for (int t = 0; t < 3; t++) {
        ...
        if ((startDist).abs() < coplanarEpsilon && (endDist).abs() < coplanarEpsilon) {
          coplanarEdge = true;
          break;
        }
        ...
        if (startDist.abs() < coplanarEpsilon) {
          continue;
        }
        ...
        if (!didIntersect && endDist.abs() < coplanarEpsilon) {
      ...
      if (!coplanarEdge && intersects == 2 && _foundEdge.distance() > coplanarEpsilon) {
        if (vertexSplitEnd != -1) {
          ...
          final singleVert = posSideVerts.length >= 2 ? negSideVerts[0] : posSideVerts[0];
