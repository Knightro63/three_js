...
class _MyAppState extends State<webgl_debug2> {
  ...
  initPage() async {
    ...
    var mat = three.MeshBasicMaterial.fromMap({"map": texture});
    var _mesh = three.Mesh(plane, mat);
    _mesh.scale.setValues(0.1, 0.1, 0.1);
    scene.add(_mesh);
    ...
    var geometry = ShapeGeometry([triangleShape]);

    var mesh = three.Mesh(
      geometry,
      three.MeshBasicMaterial.fromMap({"side": three.DoubleSide, "map": texture})
    );
