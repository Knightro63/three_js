...
class _State extends State<webgl_clipping_stencil> {
  ...
  initRenderer() {
    ...
    renderer!.setClearColor(three.Color.fromHex32(0x263238));
  ...
  createPlaneStencilGroup(geometry, plane, int renderOrder) {
    ...
    var baseMat = three.MeshBasicMaterial.fromMap({
      "depthWrite": false,
      "depthTest": false,
      "colorWrite": false,
      "stencilWrite": true,
    ...
    var mat0 = three.MeshBasicMaterial.fromMap({
      "side": three.BackSide,
    ...
    var mat1 = three.MeshBasicMaterial.fromMap({
      "side": three.BackSide,
      "clippingPlanes": List<three.Plane>.from([plane]),
  ...
  initPage() async {
    ...
    planeHelpers =
        planes.map((p) => PlaneHelper(p, 2, 0xffffff)).toList();
    for (var ph in planeHelpers) {
      ph.visible = true;
      scene.add(ph);
    }

    var geometry = TorusKnotGeometry(0.4, 0.15, 220, 60);
    object = three.Group();
    scene.add(object);
    ...
    for (int i = 0; i < 1; i++) {
      ...
      var planeMat = three.MeshStandardMaterial.fromMap({
        "color": 0xff00ff,
        "metalness": 0.1,
        "roughness": 0.75,
    ...
    var material = three.MeshStandardMaterial.fromMap({
      "color": 0xFFC107,
      "metalness": 0.1,
      "roughness": 0.75,
      "clippingPlanes": planes,
