...
class ColladaParser{
  ...
  ColladaParser(this.manager, this.text, String? path, String crossOrigin){
    ...
    textureLoader = TextureLoader(manager: manager );
	...
	Future<ColladaData?> parse() async{
		...
		void parseLibrary(XmlElement xml, String libraryName,String nodeName, parser ) {
			final library = xml.getElement(libraryName);//getElementsByTagName( xml, libraryName )[ 0 ];
      ...
      return data['build'];
		...
		List<ColladaKeyFrame> prepareAnimationData( data, defaultMatrix ) {
			...
			for ( final time in data ) {
				keyframes.add( ColladaKeyFrame(double.parse( time ), data[ time ]));
			}
		...
		Future<Map<String, dynamic>> buildAnimationChannel( channel, inputSource, outputSource ) async{
    ...
    Future<List<KeyframeTrack>> buildAnimation(Map<String,dynamic> data ) async{
      ...
      return tracks;
		...
		AnimationClip buildAnimationClip(Map<String,dynamic> data ) {
			final List<KeyframeTrack> tracks = [];
		...
		Map<String, dynamic> buildController( data ) {
    ...
    Future<List<Object3D>> buildObjects(Map<String,dynamic> geometries, instanceMaterials ) async{
      ...
      for ( final type in geometries.keys ) {
        ...
        final Material? material = ( materials.length == 1 ) ? materials[ 1 ] : GroupMaterial(materials);
      ...
      offset ??= 0;
			...
			final sourceStride = source?['stride'] ?? 0;
							...
							for ( final key in vertices.keys ) {
								...
								switch ( key ) {
									case 'POSITION':
										...
										buildGeometryData( primitive, sources[ id ], input['offset'] ?? 0, position['array'] );
		...
		parsePhysicsRigidBody(XmlElement xml, data ) {
			for (final child in xml.descendantElements) {
				switch ( child.name.local ) {
					case 'technique_common':
      ...
      return data['build'];
		...
		buildTransformList( node ) { // TODO this might be wrong
			final transforms = [];
		...
		Future<Object3D?> buildNode(Map<String,dynamic> data ) async {
			...
			for (int i = 0, l = nodes.length; i < l; i ++ ) {
				objects.add(getBuild( library['nodes']![ nodes[ i ] ], buildNode ));
			}
  ...
  // List getElementsByTagName(XmlElement xml, String name ) {
  //   final array = [];
  ...
  String parserErrorToText(XmlNode parserError ) {
    var result = '';
  ...
  List<KeyframeTrack> createKeyframeTracks( animation, List<KeyframeTrack> tracks ) {
    ...
    for (int i = 0, l = keyframes.length; i < l; i ++ ) {
      ...
      matrix.copyFromArray( value ).transpose();
