...
class BVHLoader extends Loader {
  ...
  Future<BVHObject> _parse(Uint8List bufferBytes) {
    String text = String.fromCharCodes(bufferBytes);
    return __BVHParser(text, manager, animateBoneRotations, animateBoneRotations).parse();
  }
...
class __BVHParser {
  ...
  */
  void readFrameData(List<String> data, double frameTime, bone ) {
    ...
    for (int i = 0; i < bone.children.length; i ++ ) {
      ...
      print(tokens);
			...
			if ( node['type'] != 'ENDSITE' ) {
				...
				node['channels'] = tokens..removeRange(2, numChannels);//.splice( 2, numChannels );
				node['children'] = [];
			}
