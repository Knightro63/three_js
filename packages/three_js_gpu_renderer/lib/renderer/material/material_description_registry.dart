...
class MaterialRenderState {
  MaterialRenderState({
    this.topology = gpux.GpuPrimitiveTopology.triangleList,
    this.cullMode = gpux.GpuCullMode.back,
    this.frontFace = gpux.GpuFrontFace.ccw,
    this.depthTest = true,
    this.depthWrite = true,
    this.depthCompare = gpux.GpuCompareFunction.less,
    this.depthFormat = gpux.GpuTextureFormat.depth24Plus,
    gpux.GpuColorTargetState? colorTarget
  }){
    this.colorTarget = colorTarget ?? gpux.GpuColorTargetState(format: gpux.GpuTextureFormat.bgra8Unorm);
  }

  final gpux.GpuPrimitiveTopology topology;
  ...
  final gpux.GpuTextureFormat depthFormat;
  late final gpux.GpuColorTargetState colorTarget;
  ...
  MaterialRenderState applyCommonOverrides({
    ...
    gpux.GpuPrimitiveTopology topology = gpux.GpuPrimitiveTopology.triangleList,
  }) {
    final cullModeOverride = 
      side == FrontSide? gpux.GpuCullMode.back:
      side == BackSide? gpux.GpuCullMode.front:
      gpux.GpuCullMode.none;

    final writeMask = colorWrite ? gpux.GpuColorWrite.all : 0;
  ...
  MaterialRenderState withBlend(gpux.GpuBlendState? blendState) {
    return copyWith(
  ...
  MaterialRenderState copyWith({
    gpux.GpuPrimitiveTopology? topology,
    gpux.GpuCullMode? cullMode,
    gpux.GpuFrontFace? frontFace,
    bool? depthTest,
    bool? depthWrite,
    gpux.GpuCompareFunction? depthCompare,
...
abstract class MaterialDescriptorRegistry {
  ...
  static ResolvedMaterialDescriptor? resolve(Material material, Object3D mesh) {
    ...
    var activeTopology = gpux.GpuPrimitiveTopology.triangleList;
    ...
    if (
      ...
      activeTopology = gpux.GpuPrimitiveTopology.lineList;
    } 
    else if (
      mesh is Points ||
      material is PointsMaterial
    ) {
      activeTopology = gpux.GpuPrimitiveTopology.pointList;
    }
  ...
  static void _registerDefaultsLocked() {
    final basicDescriptor = MaterialDescriptor(
      key: 'material.basic',
      shader: MaterialShaderLibrary.convert('basic'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings() + _normalBindings(),
      ...
      optionalAttributes: {GeometryAttribute.uv0},
    );
    ...
    final depthDescriptor = MaterialDescriptor(
      key: 'material.depth',
      shader: MaterialShaderLibrary.convert('depth'),
      uniformBlock: _defaultUniformBlock,
    ...
    final normalDescriptor = MaterialDescriptor(
      key: 'material.normal',
      shader: MaterialShaderLibrary.convert('normal'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings(), // Normals rarely need maps, but we give it albedo fallback bindings
      renderState: MaterialRenderState(),
      requiredAttributes: {
        GeometryAttribute.position,
        GeometryAttribute.normal,
    ...
    final toonDescriptor = MaterialDescriptor(
      key: 'material.toon',
      shader: MaterialShaderLibrary.convert('toon'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings() + _volumeBindings(),
      renderState: MaterialRenderState(),
      requiredAttributes: {
    ...
    final phongDescriptor = MaterialDescriptor(
      key: 'material.phong',
      shader: MaterialShaderLibrary.convert('phong'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings() + _volumeBindings(),
      renderState: MaterialRenderState(),
      requiredAttributes: {
    ...
    final lambertDescriptor = MaterialDescriptor(
      key: 'material.lambert',
      shader: MaterialShaderLibrary.convert('lambert'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings() + _volumeBindings(),
      renderState: MaterialRenderState(),
      requiredAttributes: {
    ...
    final shadowDescriptor = MaterialDescriptor(
      key: 'material.shadow',
      shader: MaterialShaderLibrary.convert('shadow'), // Route cleanly through your string factory converter!
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings(), 
      renderState: MaterialRenderState(),
      requiredAttributes: {
      ...
      optionalAttributes: {GeometryAttribute.normal, GeometryAttribute.uv0},
    );
    _registerInternal(shadowDescriptor, [ShadowMaterial], true);

    final lineBasicDescriptor = MaterialDescriptor(
      key: 'material.lineBasic',
      shader: MaterialShaderLibrary.convert('lineBasic'), // Hooks right into your working string factory
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings(), 
      renderState: MaterialRenderState(
        // CRITICAL OVERRIDE: Switches your hardware pipeline from rendering triangles to drawing lines
        topology: gpux.GpuPrimitiveTopology.lineList, 
      ),
      requiredAttributes: {
        GeometryAttribute.position,
        GeometryAttribute.color, // REQUIRED to protect your sequential offset registers
    ...
    final spriteDescriptor = MaterialDescriptor(
      key: 'material.sprite',
      shader: MaterialShaderLibrary.convert('sprite'), // Route through your working string conversion factory!
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings(), // Allocates albedo texture binding slots for the sprite asset maps
      renderState: MaterialRenderState(),
      requiredAttributes: {
        GeometryAttribute.position,
    ...
    final lineDashedDescriptor = MaterialDescriptor(
      key: 'material.lineDashed',
      shader: MaterialShaderLibrary.convert('lineDashed'), // Routable straight through your string factory!
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings(), 
      renderState: MaterialRenderState(
        topology: gpux.GpuPrimitiveTopology.lineList, 
    ...
    final metcapDescriptor = MaterialDescriptor(
      key: 'material.matcap',
      shader: MaterialShaderLibrary.convert('matcap'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings() + _volumeBindings(),
      renderState: MaterialRenderState(),
      requiredAttributes: {
    ...
    final distanceDescriptor = MaterialDescriptor(
      key: 'material.distance',
      shader: MaterialShaderLibrary.convert('distance'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings() + _volumeBindings(),
      renderState: MaterialRenderState(),
      requiredAttributes: {
    ...
    final shaderDescriptor = MaterialDescriptor(
      key: 'material.shader',
      shader: MaterialShaderLibrary.convert('customShader'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings(), 
      renderState: MaterialRenderState(),
      requiredAttributes: {
      ...
      optionalAttributes: {GeometryAttribute.uv0, GeometryAttribute.normal},
    );
    _registerInternal(shaderDescriptor, [ShaderMaterial], true);

    // // 2. RAW SHADER MATERIAL
    // final rawShaderDescriptor = MaterialDescriptor(
    //   key: 'material.rawShader',
    //   // Bypasses convert() caching templates entirely because the user provides 100% raw source
    //   shader: MaterialShaderLibrary.rawShader(), 
    //   uniformBlock: _defaultUniformBlock,
    //   bindings: _albedoBindings(), 
    //   renderState: MaterialRenderState(),
    //   requiredAttributes: {
    ...
    //   optionalAttributes: {GeometryAttribute.uv0, GeometryAttribute.normal},
    // );
    // _registerInternal(rawShaderDescriptor, [RawShaderMaterial], true);

    final standardDescriptor = MaterialDescriptor(
      key: 'material.pbr',
      shader: MaterialShaderLibrary.convert('pbr'),
      uniformBlock: _defaultUniformBlock,
      bindings: _albedoBindings() +
          _normalBindings() +
          _roughnessBindings() +
  ...
  static gpux.GpuBlendState? _blendStateFor(Blending mode, bool transparent, double opacity) {
  ...
  static  gpux.GpuBlendState _alphaBlend = gpux.GpuBlendState(
  ...
  static gpux.GpuBlendState _additiveBlend = gpux.GpuBlendState(
    color: gpux.GpuBlendComponent(
      srcFactor: gpux.GpuBlendFactor.srcAlpha,
  ...
  static gpux.GpuBlendState _subtractiveBlend = gpux.GpuBlendState(
    color: gpux.GpuBlendComponent(
      srcFactor: gpux.GpuBlendFactor.srcAlpha,
      dstFactor: gpux.GpuBlendFactor.one,
      operation: gpux.GpuBlendOperation.reverseSubtract,
    ),
    alpha: gpux.GpuBlendComponent(
      srcFactor: gpux.GpuBlendFactor.one,
      dstFactor: gpux.GpuBlendFactor.one,
  ...
  static gpux.GpuBlendState _multiplyBlend = gpux.GpuBlendState(
    color: gpux.GpuBlendComponent(
