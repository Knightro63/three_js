...
abstract class GeometryBuilder {
  ...
  static _VertexStreamResult _buildVertexStream(
    ...
    packedAttributes.add(_PackedAttribute(
      ...
      componentCount: 3,
      defaultValue: null,
      format: gpux.GpuVertexFormat.float32x3,
    ...
    packedAttributes.add(_PackedAttribute(
      ...
      format: gpux.GpuVertexFormat.float32x4,
    ...
    packedAttributes.add(_PackedAttribute(
      ...
      format: gpux.GpuVertexFormat.float32x2,
    ...
    final layoutAttributes = <gpux.GpuVertexAttribute>[];
    final bindings = <GeometryAttribute, GeometryAttributeBinding>{};
    int shaderLocation = 0;
    int byteOffset = 0;

    for (final attr in attributes) {
      layoutAttributes.add(gpux.GpuVertexAttribute(
        format: attr.format,
        offset: byteOffset,
      ...
      if (type != null) {
        if (!bindings.containsKey(type)) {
          bindings[type] = GeometryAttributeBinding(
            ...
            stepMode: gpux.GpuVertexStepMode.vertex,
          );
        }
      }
      shaderLocation += 1;
    ...
    final layout = gpux.GpuVertexBufferLayout(
      ...
      if (type != null) {
        if (!bindings.containsKey(type)) {
          bindings[type] = GeometryAttributeBinding(
            attribute: type,
            location: shaderLocation,
            stepMode: gpux.GpuVertexStepMode.instance,
          );
        }
      }
      shaderLocation += 1;
  ...
  static gpux.GpuVertexFormat _formatForSize(int components) {
    return switch (components) {
      1 => gpux.GpuVertexFormat.float32,
      2 => gpux.GpuVertexFormat.float32x2,
      3 => gpux.GpuVertexFormat.float32x3,
      4 => gpux.GpuVertexFormat.float32x4,
