...
class GpuBuffer {
  ...
  gpux.GpuBuffer? _gpuBuffer;

  GpuBuffer(this.device, this.descriptor);
  ...
  gpux.GpuBuffer? getBuffer() => _gpuBuffer;

  /// Returns the abstraction wrapper for this buffer if it was created.
  gpux.GpuBuffer? gpuBuffer() => _gpuBuffer;
  ...
  void bind({required int slot, required gpux.GpuRenderPassEncoder renderPass}) {
    final bufferInstance = _gpuBuffer;
    if (bufferInstance == null) return;

    // Check if GpuBufferUsage.vertex bits match
    if ((descriptor.usage & gpux.GpuBufferUsage.vertex) != 0) {
      renderPass.setVertexBuffer( slot, bufferInstance);
    } 
    // Check if GpuBufferUsage.index bits match
    else if ((descriptor.usage & gpux.GpuBufferUsage.index) != 0) {
      renderPass.setIndexBuffer(
        bufferInstance, 
        gpux.GpuIndexFormat.uint32, // Adjust format dynamically if you support 16-bit pipelines
      );
    }
  }
...
class BufferDescriptor {
  final String label;
  final int size;
  final gpux.GpuBufferUsageFlags usage;
