...
class BufferPool {
  ...
  final Map<BufferSizeClass, Queue<GpuBuffer>> _pools = {};
  final Set<GpuBuffer> _acquiredBuffers = {};
  ...
  BufferPool(this.device) {
    ...
    for (final sizeClass in BufferSizeClass.values) {
      _pools[sizeClass] = Queue<GpuBuffer>();
    }
  }

  /// Acquires a buffer from the pool or creates a new one.
  GpuBuffer acquire({required int size, required gpux.GpuBufferUsageFlags usage, String? label}) {
    ...
    if (pool.isNotEmpty) {
      ...
      final newBuffer = GpuBuffer(device, descriptor);
      newBuffer.create();
