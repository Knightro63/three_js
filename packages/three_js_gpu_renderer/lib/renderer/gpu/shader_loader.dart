...
/// T033: Shader Loader for Gpu
/// Feature: 019-we-should-not
///
/// Helper function to load WGSL shaders for Gpu.
...
String loadBasicShader() {
  ...
  return r'''
    ...
    // Compatible with Gpu (native) and Vulkan (via SPIR-V compilation)
...
/// Create basic Gpu pipeline with the embedded shader module.
///
/// @param device The active [GpuDevice] instance from your gpux backend.
/// @return [GpuRenderPipeline] or null on a creation failure.
gpux.GpuRenderPipeline? createBasicPipeline(gpux.GpuDevice device) {
  try {
    ...
    final pipelineDescriptor = gpux.GpuRenderPipelineDescriptor(
      ...
      colorTargets: [
        gpux.GpuColorTargetState(format: gpux.GpuTextureFormat.bgra8Unorm),
      ],
      primitiveTopology: gpux.GpuPrimitiveTopology.triangleList, // Maps to 'triangle-list'
    );
