...
import 'package:three_js_gpu_renderer/renderer/gpu/three_js_rendering/gpu_objects.dart';
import 'package:three_js_gpu_renderer/three_js_gpu_renderer.dart';
import 'package:three_js_math/three_js_math.dart';

class GpuShadowMap extends ShadowMap {
    ...
    for (int i = 0, il = lights.length; i < il; i++) {
      ...
      if (_shadowMapSize.x > maxTextureSize || _shadowMapSize.y > maxTextureSize) {
        if (_shadowMapSize.x > maxTextureSize) {
          _viewportSize.x = (maxTextureSize / shadowFrameExtents.x).floorToDouble();
          _shadowMapSize.x = _viewportSize.x * shadowFrameExtents.x;
          shadow.mapSize.x = _viewportSize.x;
        }

        if (_shadowMapSize.y > maxTextureSize) {
          _viewportSize.y = (maxTextureSize / shadowFrameExtents.y).floorToDouble();
