...
class GpuInfo {
  ...
  void update(count, GpuPrimitiveTopology mode, instanceCount) {
    render["calls"] = render["calls"]! + 1;

    if (mode == GpuPrimitiveTopology.triangleStrip) {
      render["triangles"] = render["triangles"]! + instanceCount * (count / 3.0);
    } else if (mode == GpuPrimitiveTopology.lineList) {
      render["lines"] = render["lines"]! + instanceCount * (count / 2);
    } else if (mode == GpuPrimitiveTopology.lineStrip) {
      render["lines"] = render["lines"]! + instanceCount * (count - 1);
    }  else if (mode == GpuPrimitiveTopology.pointList) {
