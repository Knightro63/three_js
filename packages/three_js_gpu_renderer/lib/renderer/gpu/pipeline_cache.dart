...
import 'pipeline.dart'; // Adjust based on your exact gpux library paths
...
class PipelineCache {
  final Map<PipelineKey, GpuPipeline> _cache = {};
  int _hitCount = 0;
  int _missCount = 0;
  ...
  Future<GpuPipeline> getOrCreate({
    required PipelineKey key,
    required Future<GpuPipeline> Function() factory,
  }) async {
  ...
  Future<GpuPipeline> getOrCreateFromDescriptor({
    required gpux.GpuDevice device, // Maps from original GPUDevice interface
    ...
    return getOrCreate(
      key: key,
      factory: () async {
        final pipeline = GpuPipeline(device, descriptor);
        pipeline.create();
        return pipeline;
      },
    );
  ...
  GpuPipeline? remove(PipelineKey key) {
