...
uniform VertexConfigUniforms {
    bool needsViewPosition;  // Float Index 32 (True if FLAT_SHADED, USE_BUMPMAP, or TANGENTSPACE is active)
};

// 4. PIPELINE OUTPUTS (Implicit varying matching)
// These link straight to your MeshNormal fragment shader inputs by string variable name
...
void main() {
    ...
    // Replaced web compilation macros with a unified runtime configuration flag
    if (needsViewPosition) {
        // mvPosition (ModelView position) is calculated inside project_vertex.vert
