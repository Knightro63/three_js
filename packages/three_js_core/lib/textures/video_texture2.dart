...
class VideoTexture2 extends Texture {
  ...
  factory VideoTexture2.fromOptions(
    ...
    final _controller = VideoPlayerController.asset(options.asset);//.networkUrl(Uri.parse('https://flutter.github.io/assets-for-api-docs/assets/videos/bee.mp4'))..initialize().then((_) {});
  ...
  void update(){
    ...
    if(video?.complete == true && _key != null){
      video?.complete = false;
      FlutterTexture.generateImageFromGlobalKey(_key!,video).then((value){
        video?.complete = true;
        needsUpdate = true;
      });
    }
