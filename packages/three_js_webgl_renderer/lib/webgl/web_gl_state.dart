...
class WebGLState {
  ...
  void drawBuffers(RenderTarget? renderTarget, Framebuffer? framebuffer) {
    ...
    if (needsUpdate) {
      Uint32List buf = Uint32List.fromList(List<int>.from(drawBuffers));
      gl.drawBuffers(buf);
      buf.dispose();
    }
  }
  ...
  void compressedTexImage2D(int target, int level, int internalformat, int width, int height, int border, TypedDataList? pixels) {
    gl.compressedTexImage2D(target, level, internalformat, width, height, border, pixels);
  }

  void texSubImage2D(int target, int level, int x, int y, num width, num height, int glFormat, int glType, TypedDataList data) {
  ...
  void texSubImage2DIf(int target, int level, int x, int y, int glFormat, int glType, ImageElement image) {
    if (kIsWeb && image.data is! TypedDataList) {
  ...
  void texSubImage3D(int target, int level, int xoffset, int yoffset, int zoffset, int width, int height, int depth, int format, int type, TypedDataList? pixels) {
