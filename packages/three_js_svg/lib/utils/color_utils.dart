...
extension SVGColor on Color{
  Color setStyle([String style = '', ColorSpace colorSpace = ColorSpace.srgb]) {
    ...
    if (reg1.hasMatch(style)) {
      ...
      if (size == 3) {
        // #ff0
        red = int.parse(hex[0] + hex[0], radix: 16) / 255;
        green = int.parse(hex[1] + hex[1], radix: 16) / 255;
        blue = int.parse(hex[2] + hex[2], radix: 16) / 255;

        return this;
      } else if (size == 6) {
        // #ff0000
        red = int.parse(hex[0] + hex[1], radix: 16) / 255;
        green = int.parse(hex[2] + hex[3], radix: 16) / 255;
        blue = int.parse(hex[4] + hex[5], radix: 16) / 255;
        ...
        switch (name) {
          ...
          case 'rgba':
            ...
            if (colorReg1.hasMatch(components)) {
              ...
              red = math.min(255, int.parse(c1, radix: 10)) / 255;
              green = math.min(255, int.parse(c2, radix: 10)) / 255;
              blue = math.min(255, int.parse(c3, radix: 10)) / 255;
              ...
              final colorReg2 = RegExp(
                  r"^(\d+)\%\s*,\s*(\d+)\%\s*,\s*(\d+)\%\s*(?:,\s*(\d*\.?\d+)\s*)?$");
              if (colorReg2.hasMatch(components)) {
                final match2 = colorReg2.firstMatch(components)!;
                ...
                red = math.min(100, int.parse(c1, radix: 10)) / 100;
                green = math.min(100, int.parse(c2, radix: 10)) / 100;
                blue = math.min(100, int.parse(c3, radix: 10)) / 100;
          ...
          case 'hsl':
          case 'hsla':
            final colorReg3 = RegExp(
                r"^(\d*\.?\d+)\s*,\s*(\d+)\%\s*,\s*(\d+)\%\s*(?:,\s*(\d*\.?\d+)\s*)?$");
            if (colorReg3.hasMatch(components)) {
              final match3 = colorReg3.firstMatch(components)!;
