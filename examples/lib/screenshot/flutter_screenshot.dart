...
class FlutterScreenshot extends StatefulWidget {
  const FlutterScreenshot({super.key});
  @override
  createState() => _State();
}

class _State extends State<FlutterScreenshot> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: InkWell(
        onTap: () async{
          try {
            rend.RenderRepaintBoundary? boundary = containerKey.currentContext?.findRenderObject() as rend.RenderRepaintBoundary?; 
            final image = await boundary!.toImage();
            final data = (await image.toByteData(format: ui.ImageByteFormat.png))?.buffer.asUint8List();
            SaveFile.saveBytes(printName: 'flutter_test', fileType: 'png', bytes: data!);
          }catch (e) {
            rethrow;
          }
        },
