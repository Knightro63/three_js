...
class _MyAppState extends State<WebglGeometryShapes> {
  ...
  void addShape(shape, three.ExtrudeGeometryOptions extrudeSettings, int color, double x, double y, double z, double rx, double ry, double rz, double s) {
    ...
    mesh = three.Mesh(geometry2, three.MeshPhongMaterial.fromMap({"color": color}));
  ...
  void addLineShape(three.Path shape, int color, double x, double y, double z, double rx, double ry, double rz, double s) {
    // lines
    ...
    line = three.Line(geometrySpacedPoints, three.LineBasicMaterial.fromMap({"color": color}));
    line.position.setValues(x, y, z + 25);
    line.rotation.set(rx, ry, rz);
    line.scale.setValues(s, s, s);
    group.add(line);
    ...
    particles = three.Points(geometrySpacedPoints,three.PointsMaterial.fromMap({"color": color, "size": 4}));
